...
import { rsacNeedOptions } from '@/lib/rsaConference'
...
interface RSACFormProps {
  preselected: string
  onPreselect: (value: string) => void
}

export function RSACForm({ preselected, onPreselect }: RSACFormProps) {
  ...
  if (done) {
    return (
      <div role="status" aria-live="polite" className="m3-elevated-card p-8 text-center">
        ...
        <p className="text-sm text-m3-on-surface/70">We reply within 1 business day.</p>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4" noValidate>
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          <label className={labelClass} htmlFor="rsac-name">
            Name
          </label>
          <input
            id="rsac-name"
        ...
        <div>
          <label className={labelClass} htmlFor="rsac-email">
            Email
          </label>
          <input
            id="rsac-email"
      ...
      <div>
        <label className={labelClass} htmlFor="rsac-company">
          Company
        </label>
        <input
          id="rsac-company"
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          <label className={labelClass} htmlFor="rsac-dates">
            Event date or dates
          </label>
          <input
            id="rsac-dates"
            className={inputClass}
            placeholder="Apr 6, evening"
        ...
        <div>
          <label className={labelClass} htmlFor="rsac-venue">
      ...
      <div>
        <label className={labelClass} htmlFor="rsac-need">
          What you need
        </label>
        <select
          id="rsac-need"
      ...
      <p className="text-xs text-m3-on-surface/60 text-center">We reply within 1 business day.</p>
    </form>
  )
}
