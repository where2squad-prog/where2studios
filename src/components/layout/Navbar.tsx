...
export function Navbar({ variant = 'dark', showServices = false }: NavbarProps) {
      ...
      >
        <div
          className={`w-full px-4 sm:px-6 lg:px-8 transition-all duration-300 ${
            isScrolled
              ? `py-2 ${bgColor}/95 backdrop-blur-xl border-b ${isLight ? 'border-m3-outline' : 'border-m3-on-dark/10'}`
              : `py-3 lg:py-4 ${isLight ? 'bg-transparent' : 'bg-gradient-to-b from-m3-surface-dark/70 via-m3-surface-dark/40 to-transparent'}`
          }`}
        >
            ...
            <Link to="/" className="flex items-center cursor-pointer">
              <motion.img
                ...
                className={`w-auto drop-shadow-2xl origin-left transition-all duration-300 ${
                  isScrolled ? 'h-12 sm:h-14 lg:h-16' : 'h-14 sm:h-18 lg:h-20'
                }`}
              />
            </Link>
            ...
            <div className="hidden md:flex items-center gap-1 lg:gap-2">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  to={link.href}
                  className={`m3-text-button ${textColor}/80 hover:${textColor} ${
                    location.pathname === link.href ? textColor : ''
                  }`}
                >
                  {link.label}
                </Link>
                    ...
                    <div className={`m3-elevated-card p-2 min-w-[180px]`}>
                      {serviceLinks.map((link) => (
                        <Link
                          key={link.href}
                          to={link.href}
                          className="block px-4 py-2 text-sm text-m3-on-surface hover:bg-m3-surface-variant rounded-lg"
                        >
                          {link.label}
                        </Link>
                      ))}
