...
function ProductionCard({
  ...
  return <motion.div initial={{
    ...
    delay: index * 0.05
  }} className="flex-shrink-0 w-[300px] sm:w-[360px] group cursor-pointer" onClick={() => onPlay(project)}>
...
function FilterChip({
  ...
  return <button onClick={onClick} className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200
        ${active ? 'bg-m3-primary text-m3-on-primary shadow-md' : 'bg-m3-surface-variant text-m3-on-surface hover:bg-m3-primary/10 border border-m3-outline'}`}>
      {label}
    </button>;
}
...
export function FeaturedProductions() {
  ...
  const productionProjects = useMemo(() => {
    ...
    return allProjects.filter(p => PRODUCTION_CATEGORIES.includes(p.category) && p.featured === true);
  }, [allProjects]);
  ...
  if (isLoading) {
    return <section id="featured-productions" className="py-16 sm:py-20 bg-m3-surface-variant scroll-mt-20">
        <div className="container mx-auto px-4 sm:px-8 lg:px-12">
          <div className="flex gap-4 overflow-hidden">
            {[...Array(3)].map((_, i) => <div key={i} className="flex-shrink-0 w-[300px] sm:w-[360px] aspect-video bg-m3-surface rounded-2xl animate-pulse" />)}
          </div>
        </div>
      </section>;
  }
  ...
  return <section id="featured-productions" className="py-16 sm:py-20 bg-m3-surface-variant scroll-mt-20 overflow-hidden">
      <div className="container mx-auto px-4 sm:px-8 lg:px-12">
        {/* Header */}
        <motion.div initial={{
        opacity: 0,
        ...
        once: true
          ...
          <div>
            ...
            <p className="text-m3-on-surface/60 text-sm sm:text-base mt-2 max-w-md">Corporate, events, weddings, and commercials</p>
          </div>

          {/* Navigation arrows - desktop */}
          <div className="hidden sm:flex gap-2">
            <button onClick={() => scroll('left')} className="p-2.5 rounded-full bg-m3-surface hover:bg-m3-primary/10 
                         transition-colors border border-m3-outline" aria-label="Scroll left">
              <ChevronLeft className="w-5 h-5 text-m3-on-surface" />
            </button>
            <button onClick={() => scroll('right')} className="p-2.5 rounded-full bg-m3-surface hover:bg-m3-primary/10 
                         transition-colors border border-m3-outline" aria-label="Scroll right">
        ...
        once: true
          ...
          {PRODUCTION_CATEGORIES.map(cat => <FilterChip key={cat} label={FILTER_LABELS[cat]} active={activeFilter === cat} onClick={() => handleFilterChange(cat)} />)}
        </motion.div>

        {/* Desktop: Horizontal scroll carousel */}
        <div className="hidden sm:block">
          {filteredProjects.length > 0 ? <div ref={scrollRef} className="flex gap-5 overflow-x-auto scrollbar-hide pb-4 -mx-4 px-4">
              <AnimatePresence mode="sync">
                {filteredProjects.map((project, index) => <ProductionCard key={project.id} project={project} index={index} onPlay={setSelectedProject} />)}
              </AnimatePresence>
        ...
        <div className="sm:hidden">
          {filteredProjects.length > 0 ? <div className="grid grid-cols-2 gap-3">
              <AnimatePresence mode="sync">
                {filteredProjects.slice(0, 6).map((project, index) => <motion.div key={project.id} initial={{
              opacity: 0,
              y: 20
            }} animate={{
              opacity: 1,
        ...
        {/* Video Modal */}
        <VideoModal isOpen={!!selectedProject} onClose={() => setSelectedProject(null)} videoUrl={selectedProject?.video_url || null} title={selectedProject?.title} />
