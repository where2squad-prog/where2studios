...
import { techWeekNeedOptions } from '@/lib/techWeek'
...
export function TechWeekForm({ preselected, onPreselect }: TechWeekFormProps) {
    ...
    return (
      <div role="status" aria-live="polite" className="m3-elevated-card p-8 text-center">
        ...
        <p className="text-sm text-m3-on-surface/70">
          We reply within one business day. During Tech Week week we reply same day.
        </p>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4" noValidate>
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          <label className={labelClass} htmlFor="tw-name">
            Name
          </label>
          <input
            id="tw-name"
          ...
          />
          {errors.name && <p className="text-xs text-m3-error mt-1">{errors.name}</p>}
        </div>
        <div>
          ...
          />
          {errors.email && <p className="text-xs text-m3-error mt-1">{errors.email}</p>}
        </div>
      </div>

      <div>
        <label className={labelClass} htmlFor="tw-company">
          Company
        </label>
        <input
          id="tw-company"
        ...
        />
        {errors.company && <p className="text-xs text-m3-error mt-1">{errors.company}</p>}
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          <label className={labelClass} htmlFor="tw-dates">
            Event date or dates
          </label>
          <input
            id="tw-dates"
          ...
          />
          {errors.eventDates && <p className="text-xs text-m3-error mt-1">{errors.eventDates}</p>}
        </div>
        <div>
          ...
          />
          {errors.venue && <p className="text-xs text-m3-error mt-1">{errors.venue}</p>}
        </div>
      </div>

      <div>
        <label className={labelClass} htmlFor="tw-need">
          What you need
        </label>
        <select
          id="tw-need"
        ...
        >
          ...
          {techWeekNeedOptions.map((option) => (
            <option key={option} value={option}>
              {option}
            </option>
          ))}
        </select>
        {errors.need && <p className="text-xs text-m3-error mt-1">{errors.need}</p>}
      </div>
